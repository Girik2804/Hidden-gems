...
if 'shown_alert_places' not in st.session_state:
    st.session_state.shown_alert_places = []  # For deduping proximity alerts
if 'did_show_proximity_popup' not in st.session_state:
    st.session_state.did_show_proximity_popup = False  # Only one popup per session
...
try:
    ...
    # Use live API (default behavior) - now with mood support
        ...
        opacity=0.8            # As suggested in documentation
                        ...
                        # Prototype: manual trigger for proximity alert (small right-aligned button)
                        prot_col1, prot_col2 = st.columns([5, 1])
                        with prot_col2:
                            if st.button("Trigger Alert", key="proto_proximity_btn"):
                                try:
                                    st.toast("🎉 Prototype proximity alert!", icon="🎊")
                                    st.balloons()
                                except Exception:
                                    pass
                    ...
                    # Proximity popup with confetti (live Nearby section)
                    try:
                        if not live_places_df.empty:
                            _dist_df = calculate_distance_to_places(live_places_df.copy(), user_lat, user_lon, 'lat', 'lon')
                            _near = _dist_df[_dist_df['distance_km'] <= 0.2].sort_values('distance_km').head(1)
                            if not _near.empty and not st.session_state.get('did_show_proximity_popup', False):
                                _r = _near.iloc[0]
                                _nm = _r.get('name') if 'name' in _r else (_r.get('Place Name') if 'Place Name' in _r else 'a place')
                                st.toast(f"🎉 You're {float(_r['distance_km']):.2f} km from {_nm}!", icon="🎊")
                                st.balloons()
                                st.session_state.did_show_proximity_popup = True
                    except Exception:
                        pass
...
def maybe_trigger_proximity_alerts(df, user_lat, user_lon, lat_col, lon_col, name_col, score_col=None, category_hint=None, max_distance_km=0.2):
    """Show alerts for places within max_distance_km that meet conditions.
    - If score_col provided: score >= 0.70
    - If category_hint == 'dine': alert the top nearby restaurant regardless of score
    Dedupes alerts using st.session_state.shown_alert_places.
    """
    try:
        if df is None or df.empty or user_lat is None or user_lon is None:
            return
        # compute distance if not present
        if 'distance_km' not in df.columns:
            df = calculate_distance_to_places(df.copy(), user_lat, user_lon, lat_col, lon_col)
        nearby = df[df['distance_km'] <= max_distance_km].copy() if 'distance_km' in df.columns else df.copy()
        if nearby.empty:
            return
        # Score-based alerts
        if score_col and score_col in nearby.columns:
            high_score = nearby[nearby[score_col] >= 0.70].sort_values(['distance_km', score_col], ascending=[True, False])
            for _, row in high_score.iterrows():
                place_id = f"{row[name_col]}-{round(row[lat_col],5)}-{round(row[lon_col],5)}"
                if place_id in st.session_state.shown_alert_places:
                    continue
                st.success(f"You're very close to a top-rated spot (score {row[score_col]:.2f}): {row[name_col]} — {row['distance_km']:.2f} km away. 🧭")
                st.session_state.shown_alert_places.append(place_id)
                break
        # Top nearby restaurant alert
        if category_hint == 'dine':
            top_nearby = nearby.sort_values('distance_km', ascending=True).head(1)
            if not top_nearby.empty:
                row = top_nearby.iloc[0]
                place_id = f"dine-{row[name_col]}-{round(row[lat_col],5)}-{round(row[lon_col],5)}"
                if place_id not in st.session_state.shown_alert_places:
                    st.info(f"You're near a great restaurant: {row[name_col]} — {row['distance_km']:.2f} km. 🍽️")
                    st.session_state.shown_alert_places.append(place_id)
    except Exception:
        pass
